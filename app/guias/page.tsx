...
export default function GuiasPage() {
  ...
  const fetchGuias = async () => {
    ...
    try {
      ...
      const dadosNormalizados: Guia[] = (data || []).map((item: Guia) => ({
        id: item.id,
        nome: item.nome || '',
  ...
  const salvarEdicao = async (e: React.FormEvent) => {
    ...
    try {
      const { error: updateError } = await supabase
        .from('guias')
        .update({
          nome: itemEditar.nome?.trim() || '',
          telefone: itemEditar.telefone?.trim() || '',
          idiomas: itemEditar.idiomas?.trim() || '',
          cadastur: itemEditar.cadastur?.trim() || '',
          status: itemEditar.status || 'Ativo',
        })
        .eq('id', itemEditar.id);
  ...
  const confirmarExclusao = async () => {
    ...
    try {
      ...
      setError('Erro ao excluir registro: ' + mensagem);
    } finally {
      setExcluindo(false);
    }
  };
  ...
  const guiasFiltrados = guias.filter((g) => {
    ...
    const idiomasCompleto = g.idiomas || '';
    const cadasturCompleto = g.cadastur || '';
    const statusAtual = g.status || 'Ativo';

    const atendeBusca =
      nomeCompleto.toLowerCase().includes(termo) ||
      idiomasCompleto.toLowerCase().includes(termo) ||
      cadasturCompleto.toLowerCase().includes(termo);
  ...
  const ativos = guias.filter(
    (g) => (g.status || 'Ativo').toLowerCase() === 'ativo'
  ).length;
        ...
        <div className="relative max-w-7xl mx-auto w-full z-10 pt-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            ...
            <p className="mt-1 text-emerald-100/80 text-sm drop-shadow-md font-medium">
              Gerencie a equipe de guias, registros CADASTUR, idiomas e escalas
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-col gap-3 md:flex-row">
              ...
              >
                ...
                <option value="Ativo" className="bg-[#041411]">
                  Ativo
                </option>
                <option value="Inativo" className="bg-[#041411]">
                  Inativo
                </option>
                <option value="Férias" className="bg-[#041411]">
                  Férias
                </option>
                <option value="Em Tour" className="bg-[#041411]">
                  Em Tour
                </option>
              </select>
            </div>
          ...
          <div className="mt-8 overflow-x-auto min-h-[300px]">
            {loading ? (
              <div className="p-12 text-center text-emerald-200/60 font-medium text-sm">
                Carregando guias com segurança...
              </div>
            ) : error ? (
              ...
              <table className="min-w-[1000px] w-full text-left border-collapse">
                <thead className="bg-[#051713]/80 border-b border-emerald-900/60 text-emerald-200/80 text-xs font-bold tracking-wider select-none">
                  <tr>
                    ...
                    <th className="px-5 py-3.5">IDIOMAS</th>
                    <th className="px-5 py-3.5">CADASTUR</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl bg-[#071f1a] border border-emerald-900/60 p-6 shadow-2xl space-y-4 text-center text-slate-100">
            ...
            <p className="text-xs text-emerald-200/70">
              Deseja realmente excluir o guia{' '}
