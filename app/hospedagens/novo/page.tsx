...
export default function NovaHospedagemPage() {
  ...
  const converterParaNumeroSupabase = (
    valorInput: string
  ) => {
    if (!valorInput) return 0;

    let stringValor = valorInput
      .trim()
      .replace('R$', '')
      .trim();
  ...
  const handleSubmit = async (
    ...
    try {
      const valorTratado =
        converterParaNumeroSupabase(valorDiaria);
      ...
      setMensagem(
        'Hospedagem cadastrada com sucesso! Redirecionando...'
      );
      ...
      setMensagem(
        'Erro ao cadastrar: ' +
          (err.message || 'Verifique os campos.')
      );
        ...
        <div className="relative max-w-7xl mx-auto w-full z-10 pt-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            ...
            <p className="mt-1 text-emerald-100/80 text-sm drop-shadow-md font-medium">
              ...
              nova unidade no banco de dados.
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-emerald-800/80 bg-[#041411] text-sm font-bold text-emerald-400 focus:border-emerald-500 focus:outline-none transition"
                />
              </div>
            </div>
